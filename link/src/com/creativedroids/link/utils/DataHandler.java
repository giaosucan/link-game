...
public class DataHandler {
	...
	public static int starsFoundInPack(String packName, Context context) {
		...
		String[] levelsData;
		ArrayList<String> levelList = LevelManager.sharedInstance(
				context.getResources()).findAndSelectPack(packName);
		...
		for (int i = 0; i < levelList.size() / 5; i++) {
			for (int j = 0; j < 5; j++) {
				...
				if (levelsData.length == 2) {
					...
					int movesTaken = GameSaver.sharedInstance(context)
	...
	public static int totalStarsFound(Context context) {
		...
		String[] pack = { "5x5", "6x6", "7x7", "8x8", "9x9", "10x10", "11x11",
				"12x12", "13x13", "14x14", "a5x5", "a6x6", "a7x7", "a8x8",
				"a9x9", "a10x10" };
